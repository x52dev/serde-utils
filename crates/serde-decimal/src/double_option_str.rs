...
/// Double-option string-form decimal deserializer.
///
...
where
    D: Deserializer<'de>,
{
    crate::nullable_str::deserialize(deserializer).map(Some)
}

/// Double-option string-form decimal serializer.
///
/// See [module docs](self) for more.
pub fn serialize<S>(
    ...
    S: Serializer,
{
    match value {
        ...
        Some(v) => crate::nullable_str::serialize(v, serializer),
    }
}
...
mod tests {
    ...
    struct Foo {
        ...
        #[serde(default)]
        #[serde(with = "crate::double_option_str")]
